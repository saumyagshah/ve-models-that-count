...
def get_ve_loader(mode):
    ...
    snli_dataset = SNLIVEBERTDataset(
        split=mode,
        features_dir="/mnt/data/grid/grid_features/features/",
        count_dir="/mnt/data/grid/proposals",
        snli_file_path="/mnt/data/SNLI-VE/data/",
        max_hypothesis_len=56,
        vocab_size=10000)
...
class SNLIVEDataset(data.Dataset):
    def __init__(self, split, features_dir, count_dir, snli_file_path, max_hypothesis_len, vocab=None, vocab_size=10000):
        super().__init__()
    ...
    def __getitem__(self, idx):
        ...
        count_pkl_file = os.path.join(self.count_dir, image_id + ".pickle")
        with open(count_pkl_file, 'rb') as handle:
            count_features = pickle.load(
                handle)[3].reshape(-1).astype(np.float32)
        ...
        return features, hypothesis, label, count_features, idx, q_length
...
class SNLIVEBERTDataset(data.Dataset):
    def __init__(self, split, features_dir, count_dir, snli_file_path, max_hypothesis_len, vocab=None, vocab_size=10000):
        super().__init__()
    ...
    def __getitem__(self, idx):
        ...
        # return {"features": features, "bboxes": bb_boxes, "hypothesis": hypothesis, "label": label}
        # v, q, a, b, item, q_length
        return features, hypothesis_text, label, count_features, idx, q_length
