...
import config
import data_grid_ve
import model_grid_noc_arch_change as model_arch_change
import utils
        ...
        if has_answers:
            ...
            if train:
                ...
                if best_train_acc is None or acc_log > best_train_acc:
                    ...
                    torch.save(net.state_dict(), "model_noc_our_tf1_best_train_acc.pt")
            else:
                acc_log = acc.mean().item()
                writer.add_scalar("val/acc", acc_log, cnt_iter_val)
                if best_val_acc is None or acc_log > best_val_acc:
                    best_val_acc = acc_log
                    torch.save(net.state_dict(), "model_noc_our_tf1_best_val_acc.pt")
            fmt = '{:.4f}'.format
...
def main():
    ...
    if args.test:
        args.eval_only = True
    src = open('model_grid_noc_arch_change.py').read()
    if args.name:
        name = ' '.join(args.name)
    else:
