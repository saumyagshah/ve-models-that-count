...
def run(net, loader, optimizer, scheduler, tracker, train=False, has_answers=True, prefix='', epoch=0):
    ...
    for v, q, a, b, idx, q_len in loader:
        ...
        if train:
            ...
            # wandb.log({"train/loss": loss.item(), "train/acc": torch.mean(acc.squeeze())})
        else:
            # wandb.log({"val/loss": loss.item(), "val/acc": torch.mean(acc.squeeze())})
...
def main():
    ...
    if args.test:
        args.eval_only = True
    src = open('model_no_counting.py').read()
    if args.name:
        name = ' '.join(args.name)
    else:
    ...
    net = model_no_counting.Net(val_loader.dataset.num_tokens).cuda()
