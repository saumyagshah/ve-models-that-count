...
class Counter(nn.Module):
    ...
    def forward(self, boxes, attention):
        ...
        boxes, attention = self.filter_most_important(
            self.objects, boxes, attention)
        # normalise the attention weights to be in [0, 1]
        if not self.already_sigmoided:
            attention = F.sigmoid(attention)

        # with open('bboxes.pickle', 'wb+') as handle:
        #     pk.dump(boxes.clone().cpu().detach().numpy(),
        #             handle, protocol=pk.HIGHEST_PROTOCOL)

        relevancy = self.outer_product(attention)
        distance = 1 - self.iou(boxes, boxes)

        # with open('iou_distance.pickle', 'wb+') as handle:
        #     pk.dump(distance.clone().cpu().detach().numpy(),
        #             handle, protocol=pk.HIGHEST_PROTOCOL)
        ...
        dedup_per_entry, dedup_per_row = self.deduplicate(
            dedup_score, attention)
        ...
        # count_mat = score[0] + torch.diag(correction[0])

        score = score.sum(dim=2).sum(dim=1, keepdim=True) + \
        ...
        conf = self.f[7](att_conf.mean(dim=1, keepdim=True) +
    ...
    def to_one_hot(self, scores):
        ...
        target_r.scatter_(dim=1, index=(
            i + 1).clamp(max=self.objects), value=1)
    ...
    def filter_most_important(self, n, boxes, attention):
        ...
        idx = idx.unsqueeze(dim=1).expand(
            boxes.size(0), boxes.size(1), idx.size(1))
    ...
    def iou(self, a, b):
        ...
        # print(torch.isnan(inter).sum())
        # with open("iou_logs", "w+") as f:
        # f.write(str(torch.isnan(inter).sum()))
        area_a = self.area(a).unsqueeze(2).expand_as(inter)
        # print(torch.isnan(area_a).sum())
        # with open("iou_logs", "a") as f:
        # f.write(str(torch.isnan(area_a).sum()))
        area_b = self.area(b).unsqueeze(1).expand_as(inter)
        # print(torch.isnan(area_b).sum())
        # with open('iou_logs.pickle', 'wb+') as handle:
        #     pk.dump(
        #         [inter],
        #         handle,
...
class PiecewiseLin(nn.Module):
    ...
    def forward(self, x):
        ...
        # print("Idx: ", idx.shape, "Csum: ", csum.shape)
        # with open('idx_tensor.pickle', 'wb+') as handle:
        #     pk.dump(idx.clone().cpu().detach().numpy(),
        #             handle, protocol=pk.HIGHEST_PROTOCOL)
        # with open('idx_tensor_clamped.pickle', 'wb+') as handle:
        #     pk.dump((idx.clamp(max=self.n)).clone().cpu().detach(
        #     ).numpy(), handle, protocol=pk.HIGHEST_PROTOCOL)
