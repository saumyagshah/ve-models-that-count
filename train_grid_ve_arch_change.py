...
import config
import data_grid_ve
import model_grid_arch_change as model_arch_change
import utils
...
def run(net, loader, optimizer, scheduler, tracker, train=False, has_answers=True, prefix='', epoch=0):
    ...
    acc_tracker = tracker.track('{}_acc'.format(
        prefix), tracker_class(**tracker_params))
    for v, q, a, co, idx, q_len in loader:
        var_params = {
            'volatile': not train,
            'requires_grad': False,
        }
        ...
        co = Variable(co.cuda(non_blocking=True), **var_params)
        q_len = Variable(q_len.cuda(non_blocking=True), **var_params)

        out = net(v, co, q, q_len)
        if has_answers:
            ...
            if train:
                ...
                if best_train_acc is None or acc_log > best_train_acc:
                    ...
                    torch.save(net.state_dict(),
                               "model_wc_our_tf1_best_train_acc.pt")
            else:
                acc_log = acc.mean().item()
                ...
                if best_val_acc is None or acc_log > best_val_acc:
                    ...
                    torch.save(net.state_dict(),
                               "model_wc_our_tf1_best_val_acc.pt")
            fmt = '{:.4f}'.format
...
def main():
    ...
    if args.test:
        args.eval_only = True
    src = open('model_grid_arch_change.py').read()
    if args.name:
        name = ' '.join(args.name)
    else:
    ...
    if not args.eval_only:
        ...
        train_loader = data_grid_ve.get_ve_loader(mode="train")
    if not args.test:
        val_loader = data_grid_ve.get_ve_loader(mode="val")
    # else:
    ...
    net.load_state_dict(torch.load(
        "analysis_counting/models/transformer_naive_no_bbox_count_module/counting/model_wc_our_tf1_best_val_acc.pt"))
    ...
    co = torch.unsqueeze(torch.tensor(
        train_loader.dataset[58408][3]), dim=0).cuda()
    q_len = torch.tensor(train_loader.dataset[58408][5]).cuda()
    tmp_output = net(v, co, q, q_len)
